...
# s3 자료실을 구현할 폴더 생성
mkdir -p  /root/student_mgr/static

# root/student_mgr 폴더의 권한을 모든 사용자가 읽고 쓰고 실행 가능하도록 설정
chmod -R 777 /root/student_mgr;
...
cp /root/student_mgr.conf /etc/nginx/sites-available;
...
sed 's/localhost/'"$my_ip"'/' -i /etc/nginx/sites-available/student_mgr.conf ;
...
# nginx 설정
ln -s /etc/nginx/sites-available/student_mgr.conf;
